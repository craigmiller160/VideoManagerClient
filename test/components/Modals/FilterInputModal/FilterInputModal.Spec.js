...
import { ADD_ACTION, EDIT_ACTION } from 'store/filterInputModal/filterInputModal.constants';
...
describe('FilterInputModal', () => {
    ...
    it('renders correctly', () => {
        ...
        expect(component.find('Button').at(0).props()).toEqual(expect.objectContaining({
            color: 'success',
            type: 'submit'
        }));
        expect(component.find('Button').at(0).find('button').text()).toEqual('Save');
        expect(component.find('Button').at(1).props()).toEqual(expect.objectContaining({
            color: 'info',
            type: 'button'
        }));
        expect(component.find('Button').at(1).find('button').text()).toEqual('Cancel');
    });
    ...
    it('renders special edit action items', () => {
        ...
        expect(component.find('h5.modal-title').text()).toEqual('Edit Category');
        ...
        expect(component.find('Form').at(0).props()).toEqual(expect.objectContaining({
            initialValues: {
                id: value.value,
                name: value.label
            }
