...
const testRendering = (
	component,
	{ directory = false, selectHidden = false, directoriesOnly = false } = {}
) => {
	...
	expect(fileListItem.find('img').props()).toEqual({
		src: directory ? 'fileMock' : 'fileMock',
	...
	if (directory) {
		expect(fileListItem.find('Button[data-name="open-btn"]')).toHaveLength(
			1
		);
		expect(
			fileListItem.find('Button[data-name="open-btn"]').props()
		).toEqual(
		...
		expect(
			fileListItem.find('Button[data-name="open-btn"]').text()
		).toEqual('Open');
	}

	if ((directory && directoriesOnly) || (!directory && !selectHidden)) {
		expect(
			fileListItem.find('Button[data-name="select-btn"]')
		).toHaveLength(1);
		expect(
			fileListItem.find('Button[data-name="select-btn"]').props()
		).toEqual(
			expect.objectContaining({
		...
		expect(
			fileListItem.find('Button[data-name="select-btn"]').text()
		).toEqual('Select');
	}
};
