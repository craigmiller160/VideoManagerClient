...
describe('Input', () => {
	...
	it('should render InnerComponent as input', () => {
		const component = mount(<InnerComponent type="text" />);
	...
	it('is invisible if type is hidden', () => {
		const component = mount(<InputComponent {...props} type="hidden" />);
		expect(component.find('StyledFormGroupDiv')).toHaveStyleRule(
			'display',
			'none'
		);
	...
	it('renders textarea', () => {
		...
		const component = mount(<InputComponent {...newProps} />);
		...
		expect(component.find('StyledTextArea')).toHaveStyleRule(
			'resize',
			'none'
		);
	});
