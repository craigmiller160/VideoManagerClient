...
describe('Pagination', () => {
	it('should render correctly', () => {
		...
		const component = mount(
			<Pagination {...getProps(1)} onClick={clickHandler} />
		);
		...
		expect(
			paginationItems.at(0).find('PaginationLink').props().first
		).toEqual(true);
		expect(
			paginationItems.at(1).find('PaginationLink').props().previous
		).toEqual(true);
		expect(
			paginationItems.at(7).find('PaginationLink').props().next
		).toEqual(true);
		expect(
			paginationItems.at(8).find('PaginationLink').props().last
		).toEqual(true);
	...
	it('should only render with next button', () => {
		const component = mount(<Pagination {...getProps(0)} />);
		...
		expect(
			paginationItems.at(5).find('PaginationLink').props().next
		).toEqual(true);
		expect(
			paginationItems.at(6).find('PaginationLink').props().last
		).toEqual(true);
		expect(paginationItems.at(0).props().active).toEqual(true);
	});

	it('should only render with previous button', () => {
		const component = mount(<Pagination {...getProps(4)} />);
	...
	describe('the max number of buttons limit', () => {
		it('is limited when the currentPage is in the middle', () => {
			const component = mount(<Pagination {...getProps(10, 22)} />);
		...
		it('is limited when the currentPage is at the beginning', () => {
			const component = mount(<Pagination {...getProps(0, 22)} />);
		...
		it('is limited when the currentPage is at the end', () => {
			const component = mount(<Pagination {...getProps(21, 22)} />);
	...
	describe('creates the correct number of page buttons', () => {
		...
		const mountPageBtns = (currentPage) =>
			mount(
				<Fragment>
