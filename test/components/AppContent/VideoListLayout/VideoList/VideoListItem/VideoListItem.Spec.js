...
const defaultProps = {
	videoFile: {
		fileId: 1,
		expanded: false,
		series: [{ seriesId: 1, seriesName: 'seriesName' }],
		categories: [{ categoryId: 1, categoryName: 'categoryName' }],
		stars: [{ starId: 1, starName: 'starName' }],
		fileName: 'fileName',
...
const testRendering = (
	component,
	{ isExpanded = false, hasDisplayName = true, hasEditRole = true } = {}
) => {
	const rootDiv = component.find(
		`div[data-name="video-list-item-${defaultProps.videoFile.fileId}"]`
	);
	...
	const findLabelSpan = (dataName, value) =>
		expect(component.find(`span[data-name="${dataName}"]`).text()).toEqual(
			value
		);
	const findTextItem = (dataName, value) =>
		expect(
			component.find(`ListGroupItemText[data-name="${dataName}"]`).text()
		).toEqual(value);
	const findP = (dataName, value) =>
		expect(
			component.find(`Collapse p[data-name="${dataName}"]`).text()
		).toEqual(value);

	expect(rootDiv).toHaveLength(1);
	expect(rootDiv.props()).toEqual(
	...
	if (isExpanded) {
		...
		expect(component.find('Button[data-name="play-btn"]').text()).toEqual(
			'Play'
		);
		if (hasEditRole) {
			expect(
				component.find('Button[data-name="edit-btn"]').text()
			).toEqual('Edit');
		}
	}
};
	...
	describe('Callbacks and actions', () => {
		it('dispatches expandVideoFile', () => {
			...
			component
				...
				.props()
				.onClick();
			expect(store.getActions()).toEqual([{ type: 'EXPAND_VIDEO_FILE' }]);
		});
		...
		it('calls playVideoClick', async () => {
			...
			await component
				.find('Button[data-name="play-btn"]')
				.props()
				.onClick();
			expect(store.getActions()).toEqual([{ type: 'RESET' }]);
			expect(open).toHaveBeenCalledWith(
				`/video-manager/play/${defaultProps.videoFile.fileId}`,
				'_blank'
			);
		});
	});
