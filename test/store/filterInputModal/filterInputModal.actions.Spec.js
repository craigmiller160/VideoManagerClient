...
import {
	BASE_STAR_FILTERS,
	NEW_STAR_FILTER
} from '../../exclude/mock/mockData/starData';
...
describe('filterInputModal.actions', () => {
	...
	it('showEditCategoryModal action', () => {
		const expectedAction = {
			type: showEditCategoryModal.toString(),
			payload: 1
		};
	...
	it('showEditSeriesModal action', () => {
		const expectedAction = {
			type: showEditSeriesModal.toString(),
			payload: 1
		};
	...
	it('showEditStarModal action', () => {
		const expectedAction = {
			type: showEditStarModal.toString(),
			payload: 1
		};
	...
	describe('saveFilterChanges action', () => {
		...
		const getStateFormValues = (type) => {
			switch (type) {
				case CATEGORY_TYPE:
					return NEW_CATEGORY_FILTER;
				case SERIES_TYPE:
					return NEW_SERIES_FILTER;
				case STAR_TYPE:
					return NEW_STAR_FILTER;
				default:
		...
		it('fails if cannot find form', async () => {
			...
			try {
				...
				expect(ex.message).toContain(
					'Cannot find filterInputForm in Redux store'
				);
				return;
			}
		...
		it('add new category', async () => {
			...
			const expectedActions = [
				{
					...
					payload: BASE_CATEGORY_FILTERS
				},
				{
					type: showSuccessAlert.toString(),
					payload: 'Successfully saved Category filter'
				}
			];
			...
			expect(mockApi.history).toEqual(
		...
		it('add new series', async () => {
			...
			const expectedActions = [
				...
				{
					type: showSuccessAlert.toString(),
					payload: 'Successfully saved Series filter'
				}
			];
			try {
		...
		it('add new star', async () => {
			...
			const expectedActions = [
				...
				{
					type: showSuccessAlert.toString(),
					payload: 'Successfully saved Star filter'
				}
			];
			try {
	...
	describe('deleteFilter action', () => {
		const createState = (type) => ({
			...
			videoSearch: {
				filters: {
					categories:
						BASE_CATEGORY_FILTERS.concat(NEW_CATEGORY_FILTER),
			...
			const expectedActions = [
				{
					...
					payload: 'Invalid state for deleting a filter'
				}
			];
			try {
		...
		it('deletes category', async () => {
			...
			const expectedActions = [
				...
				{
					type: showSuccessAlert.toString(),
					payload: 'Successfully deleted Category filter'
				}
			];
			try {
				await store.dispatch(deleteFilter());
		...
		it('deletes series', async () => {
			...
			const expectedActions = [
				...
				{
					type: showSuccessAlert.toString(),
					payload: 'Successfully deleted Series filter'
				}
			];
			try {
				await store.dispatch(deleteFilter());
		...
		it('deletes star', async () => {
			...
			const expectedActions = [
				...
				{
					type: showSuccessAlert.toString(),
					payload: 'Successfully deleted Star filter'
				}
			];
			try {
				await store.dispatch(deleteFilter());
