...
const VideoPlayerPage = (props) => {
	...
	const { loading, videoFile, videoToken } = useSelector(
	...
	const formattedLastViewed = videoFile.lastViewed
		...
		: '';
	const formattedFileAdded = videoFile.fileAdded
		? new VideoDate(videoFile.fileAdded).formatDateTime()
		: '';

	return (
		<>
			<Helmet title={getFileName(videoFile)} />
